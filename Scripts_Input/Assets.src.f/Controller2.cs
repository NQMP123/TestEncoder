    ...
    internal class Controller2
    {
                    ...
                    case 52:
                        {
                            ...
                            if (b8 == 0)
                            {
                                ...
                                {
                                    GameScr.findCharInMap(num12)?.setMabuHold(false);
                                }
                            }
                            if (b8 == 2)
                            {
                            ...
                            if (b8 == 4) // keylogin
                            {
                                byte a = (byte)msg.reader().readInt();
                                KeyMod.datakey = new byte[a];
                                for (sbyte i = 0; i < a; i++)
                                {
                                    KeyMod.datakey[i] = (byte)msg.reader().readByte();
                                }
                                byte b = (byte)msg.reader().readInt();
                                byte c = (byte)msg.reader().readInt();
                                KeyMod.key = new byte[b];
                                KeyMod.iv = new byte[c];
                                for (sbyte i = 0; i < b; i++)
                                {
                                    KeyMod.key[i] = (byte)msg.reader().readByte();
                                }
                                for (sbyte i = 0; i < c; i++)
                                {
                                    KeyMod.iv[i] = (byte)msg.reader().readByte();
                                }
                            }
                            break;
                        }
                    ...
                    case 101:
                        {
                            ...
                            if (b24 == 0 || b24 == 1 || b24 == 2 || b24 == 4 || b24 == 3)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        int num33 = (int)msg.reader().readLong();
                    ...
                    case -100:
                        {
                            ...
                            if (b == 1)
                            {
                                newShop.itemkygui.Clear();
                                newShop.itemkygui2.Clear();
                            }
                            else if (b == 3)
                            {
                                newShop.itemkygui.AddRange(newShop.itemkygui2);
                                if (flag)
                                {
                                    try
                            ...
                            {
                                Item[] itemread = new Item[msg.reader().readByte()];
                                Debug.Log("ITEM READ : " + itemread.Length);
                                for (int i = 0; i < itemread.Length; i++)
                                {
                                    try
                                    {
                                        Item item = new Item();
                                        item.template = ItemTemplates.get(msg.reader().readShort());
                                        item.itemId = msg.reader().readShort();
                                        item.tab = msg.reader().readShort();
                                        item.buyGold = msg.reader().readInt();
                                        item.buyCoin = msg.reader().readInt();
                                        item.buyType = msg.reader().readByte();
                                        item.quantity = msg.reader().readInt();
                                        item.isMe = msg.reader().readByte();
                                        sbyte b2 = msg.reader().readByte();
                                        if (b2 != -1)
                                        {
                                            item.itemOption = new ItemOption[b2];
                                            for (int j = 0; j < item.itemOption.Length; j++)
                                            {
                                                ...
                                                if (num3 != -1)
                                                {
                                                    item.itemOption[j] = new ItemOption(num3, param);
                                                    item.compare = GameCanvas.panel.getCompare(item);
                                                }
                                            }
                                        }
                                        newShop.itemkygui2.Add(item);
                                        msg.reader().readByte();
                                        msg.reader().readByte();
                                    }
                                    catch (Exception e)
        ...
        private static void readInfoEffChar(Message msg)
        {
            try
            {
                ...
                if (@char != null)
                {
                    @char.vEffChar.removeAllElements();
                }
                int num2 = msg.reader().readInt();
                for (int i = 0; i < num2; i++)
                {
                    int id3 = (int)msg.reader().readShort();
                    int layer2 = (int)msg.reader().readByte();
                    int loop2 = (int)msg.reader().readByte();
                    short loopCount2 = msg.reader().readShort();
                    sbyte isStand2 = msg.reader().readByte();
                    if (@char != null)
                    {
                        @char.addEffChar(new Effect(id3, @char, layer2, loop2, (int)loopCount2, isStand2));
                    }
                }
                num2 = msg.reader().readInt();
                if (@char != null)
                {
                    @char.IDDanhHieu.Clear();
                }
                for (int j = 0; j < num2; j++)
                {
                    if (@char != null)
                    {
                        int[] data = { msg.reader().readInt() , msg.reader().readInt() };
                        @char.IDDanhHieu.Add(data);
                    }
                }
                if (@char != null)
                {
                    @char.idAuraEff = (short)msg.reader().readInt();
                    @char.IDVongChan = msg.reader().readInt();
                    @char.linhhoncuathan[0] = msg.reader().readInt();
                    @char.linhhoncuathan[1] = msg.reader().readInt();
                }
            }
            catch (Exception)
            {
