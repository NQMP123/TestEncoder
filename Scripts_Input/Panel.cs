...
public class Panel : IActionListener, IChatable
{
    ...
    public static string specialInfopet;

    public static short spearcialImagepet;
    ...
    public static int WIDTH_PANEL = 350;
    ...
    public static byte chisocongthem;
    ...
    ComboBoxbyNQMP comboSort = new ComboBoxbyNQMP(20, 150, 200, 20, 10, "Tất Cả");
    public Panel()
    {
        init();
    ...
    private void setType(int position)
    {
        ...
        cmxLast = new int[currentTabName.Length];
        for (int i = 0; i < currentTabName.Length; i++)
        {
            lastSelect[i] = (GameCanvas.isTouch ? (-1) : 0);
        }
        comboPanel.x = xScroll;
        comboPanel.w = wScroll;
        comboPanel.h = 20;
        for (int i = 0; i < comboPanel.length; i++)
        {
            comboPanel.y[i] = yScroll + i * 20;
        }
        ...
        comboSort.SetBounds(this.xScroll, this.yScroll, this.wScroll, 20);
        comboSort.ClearItems();
        comboSort.AddItem("Tất Cả", 0);
        comboSort.AddItem("Ngọc rồng", 1);
        comboSort.AddItem("Cải trang", 2);
        comboSort.AddItem("Trang bị", 3);
        comboSort.AddItem("Bổ trợ", 4);
        comboSort.AddItem("Khác", 5);
        comboSort.AddItem("Sắp xếp", 6);
        comboSort.action = delegate ()
        {
            int index = comboSort.GetSelectedIndex();
            if (index == 6)
            {
                Service.gI().SortItemBag();
    ...
    public void setTabKiGui()
    {
        ...
        {
            ITEM_HEIGHT = 24;
            currentListLength = newShop.itemkyguiTab(newShop.currentTab).Length;
            Debug.Log("Current : " + currentListLength);
            cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
            if (cmyLim < 0)
            {
                cmyLim = 0;
    ...
    public void addItemDetail(Item item)
    {
        try
        {
            ...
            {
                text += "\n--\n";
                text += !getLevelItem(13) ? "|0|" : "|1|";
                text += "(5 Món Thần Linh trở lên) +3% Chỉ Số\n";
                text += !getLevelItem(14) ? "|0|" : "|1|";
                text += "(5 Món Hủy Diệt trở lên) +5% Chỉ Số\n";
                text += !getLevelItem(15) ? "|0|" : "|1|";
                text += "(5 Món Thiên Sứ trở lên) +10% Chỉ Số\n";
            }
            if (item.template.type < 5)
            {
                text += "\n--\n";
                text += !getDoneLevel(6) ? "|0|" : "|1|";
                text += "(5 Món +5 trở lên) +3% Chỉ Số\n";
                text += !getDoneLevel(6) ? "|0|" : "|1|";
                text += "(5 Món +6 trở lên) +5% Chỉ Số\n";
                text += !getDoneLevel(7) ? "|0|" : "|1|";
                text += "(5 Món +7 trở lên) +7% Chỉ Số\n";
                text += !getDoneLevel(8) ? "|0|" : "|1|";
                text += "(5 Món +8 trở lên) +10% Chỉ Số\n";
            }
    ...
    public bool getLevelItem(int level)
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            if (i < 5)
            {
                ...
                if (item != null && item.template.type <= 4 && item.template.level < level)
                {
                    return false;
                }
            }
    ...
    public bool getDoneLevel(int level)
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            if (i < 5)
            {
                ...
                if (item != null && item.template.type <= 4 && FindParam(item, 72) < level)
                {
                    return false;
                }
            }
    ...
    public static int FindParam(Item item, int id)
    {
        if (item == null || id == -1)
        {
            return -1;
        }
        foreach (ItemOption itemOption in item.itemOption)
        {
        ...
        else
        {
            ...
            else
            {
                ...
                if (Equals(GameCanvas.panel) && GameCanvas.panel2 == null && GameCanvas.isPointerJustRelease && !GameCanvas.isPointer(X, Y, W, H) && !pointerIsDowning)
                {
                    ...
                    if (showSort)
                    {
                        if (Panel.cmdSort.isPointerPressInside())
                        {
                            flag = true;
                        }
                        for (int i = 0; i < cmdSortShow.Length; i++)
                        {
                            if (cmdSortShow[i].isPointerPressInside())
                            {
                                flag = true;
                            }
                        }
    ...
    {
        isClanOption = false;
        if (type != 0 || mainTabName.Length != 6 || currentTabIndex != 4)
        {
            return;
        }
        isClanOption = false;
    ...
    private void updateKeyScrollView()
    {
        ...
        bool flag = false;
        bool flag2 = type == 0 && currentTabIndex == 1;
        bool flag3 = (GameCanvas.panel2 == null && type == 0 && currentTabIndex == 2) || type == 7;
        if (flag2 || flag3)
        {
            if (flag2)
            {
            ...
            else if (flag3)
            {
                ...
                if (check)
                {
                    ...
                    if (selected > Char.myCharz().arrItemBag.Length)
                    {
                        selected = -1;
                    }
                    flag = true;
        ...
        if (flag)
        {
            ...
            {
                cmtoY = (selected / 5) * ITEM_HEIGHT - hScroll / 2;
                if (cmtoY > cmyLim)
                {
                    cmtoY = cmyLim;
                }
        ...
        if (GameCanvas.isPointerDown)
        {
            ...
            {
                pointerDownTime++;
                if (pointerDownTime > 5 && pointerDownFirstX == GameCanvas.py && !isDownWhenRunning && !flag3)
                {
        ...
        if (Res.abs(i2) < 20 && Res.abs(GameCanvas.py - pointerDownFirstX) < 20 && !isDownWhenRunning && !flag3)
        {
            cmRun = 0;
            cmtoY = cmy;
            pointerDownFirstX = -1000;
    ...
    {
        currentListLength = Char.myCharz().arrItemBody.Length;
        ITEM_HEIGHT = 24;
        cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
        cmy = (cmtoY = cmyLast[currentTabIndex]);
        if (cmyLim < 0)
    ...
    int pickX;
    private void updateKeyScrollInven()
    {
        ...
        // Cập nhật offset từ mScroll
        if (mscroll != null)
        {
            RectInt viewRect = new RectInt(xScroll, yScroll, wScroll, hScroll);
            Vector2Int delta = mscroll.UpdateKey(viewRect);
            scrollOffset -= delta.x; // Cộng dồn delta.x
                                     // Giới hạn scrollOffset
            int maxOffset = currentListLength * 30 - wScroll;
            if (scrollOffset < 0)
            {
                scrollOffset = 0;
            }
            ...
            Console.WriteLine($"paintInventory2: delta.x={delta.x}, scrollOffset={scrollOffset}, maxOffset={maxOffset}");
        }
        ...
        if (flag2 || flag3)
        {
            if (flag2)
            {
                if (GameCanvas.keyPressed[(!Main.isPC) ? 4 : 23]) // Trái
                {
                    selected--;
                    flag = false;
                }
                if (GameCanvas.keyPressed[(!Main.isPC) ? 6 : 24]) // Phải
                {
                    selected++;
                    flag = false;
                }
            ...
            else if (flag3)
            {
                ...
                if (GameCanvas.keyPressed[(!Main.isPC) ? 2 : 21]) // Lên
                {
                    selected -= 6;
                    check = true;
                }
                if (GameCanvas.keyPressed[(!Main.isPC) ? 8 : 22]) // Xuống
                {
                    selected += 6;
                    check = true;
                }
                if (check)
                {
                    ...
                    if (selected >= Char.myCharz().arrItemBag.Length)
                    {
                        selected = Char.myCharz().arrItemBag.Length - 1;
                    }
                    flag = true;
                }
            }
        ...
        else
        {
            ...
            else if (GameCanvas.keyPressed[(!Main.isPC) ? 6 : 24]) // Phải
            {
                flag = true;
                selected++;
                if (selected >= Char.myCharz().arrItemBag.Length)
        ...
        if (flag)
        {
            Console.WriteLine($"updateKeyScrollInven: selected={selected}");
        }
        if (GameCanvas.menu.showMenu)
        {
            return;
        ...
        if (GameCanvas.isPointerHoldIn(xScroll, yScroll+ ITEM_HEIGHT, wScroll, hScroll- ITEM_HEIGHT) && GameCanvas.isPointerClick)
        {
            int ITEM_WIDTH = 30;
            int column = (((scrollOffset + GameCanvas.px) - xScroll) / ITEM_WIDTH);
            Debug.LogError("column : " + column);

            int row = ((GameCanvas.py - yScroll - 20) / ITEM_HEIGHT);
            Debug.LogError("row : " + row);

            int selectNew = row + (column * 6);
            Debug.LogError("selectNew : " + selectNew);
            ...
            isClick = true;
            xPick = GameCanvas.px;
            lastPick = mSystem.currentTimeMillis();
        }
        if (isClick && !GameCanvas.isPointerDown)
        {
            checkOptionSelect();
            pointerDownTime = 0;
            waitToPerform = 3;
        ...
        if (isClick && GameCanvas.isPointerMove)
        {
            selected = -1;
            isClick = false;
        }
    ...
    long lastPick;
    private mScroll mscroll; // Thêm biến mScroll
    bool isClick = false;
    int xPick = 0;
    private void setTabInventory(bool resetSelect)
    {
        SortItemBag = 0;
        showSort = false;
        // Tính số cột dựa trên số item, mỗi cột 6 ô
        currentListLength = (Char.myCharz().arrItemBag.Length + 5) / 6; // Số cột
        ITEM_HEIGHT = 28;
        int ITEM_WIDTH = 30; // Chiều rộng mỗi cột (từ paintInventory2)
        cmxLim = currentListLength * ITEM_WIDTH - wScroll; // Giới hạn cuộn ngang
        if (cmxLim < 0)
        {
            cmxLim = 0;
        }
        cmx = cmxLast[currentTabIndex]; // Khởi tạo vị trí cuộn
        if (cmx < 0)
        {
            cmx = 0;
        }
        ...
        // Khởi tạo mScroll
        mscroll = new mScroll(ScrollType.Horizontal,
            eventDown: () => Console.WriteLine("Scroll Down"),
            eventMove: () => Console.WriteLine("Scroll Move"),
            eventEnd: () => Console.WriteLine("Scroll End"));
        Debug.Log($"setTabInventory: currentListLength={currentListLength}, cmxLim={cmxLim}, cmx={cmx}");
    }
    ...
    private void paintShop(mGraphics g)
    {
        ...
        if (typeShop == 2 && Equals(GameCanvas.panel))
        {
            if (currentTabIndex <= 3 && GameCanvas.isTouch)
            {
                if (cmy < -50)
                {
                    GameCanvas.gI().paintLoading(g, true);
                }
                else if (cmy < 0)
                {
                ...
                if (item.buyCoin != 0 || item.buyGold != 0)
                {
                    ...
                    if (typeShop == 2 && currentTabIndex <= 3 && !Equals(GameCanvas.panel2))
                    {
                        if (item.buyCoin > 0 && item.buyGold > 0)
                        {
                            ...
                            {
                                g.drawImage(imgXu, num2 + num4 - 7, num3 + 7, 3);
                                mFont.tahoma_7b_yellow.drawString(g, Res.formatNumber2(item.buyCoin), num2 + num4 - 15, num3 + 1, mFont.RIGHT);
                            }
                            if (item.buyGold > 0)
                            {
                                g.drawImage(imgLuong, num2 + num4 - 7, num3 + 7 + 11, 3);
                                mFont.tahoma_7b_green.drawString(g, Res.formatNumber2(item.buyGold), num2 + num4 - 15, num3 + 12, mFont.RIGHT);
                            }
                        }
                        else
                        {
                            ...
                            {
                                g.drawImage(imgLuong, num2 + num4 - 7, num3 + 7, 3);
                                mFont.tahoma_7b_green.drawString(g, Res.formatNumber2(item.buyGold), num2 + num4 - 15, num3 + 1, mFont.RIGHT);
                            }
                        }
                    }
                }
    ...
    private void paintPetStatus(mGraphics g)
    {
        ...
        for (int i = 0; i < strStatus.Length; i++)
        {
            ...
            if (num - cmy <= yScroll + hScroll && num - cmy >= yScroll - ITEM_HEIGHT)
            {
                ...
                mFont.tahoma_7b_dark.drawString(g, strStatus[i], xScroll + 10, num + 6, mFont.LEFT);
            }
        }
        paintScrollArrow(g);
    }
    ...
    private void paintTools(mGraphics g)
    {
        ...
        for (int i = 0; i < strTool.Length; i++)
        {
            ...
            mFont.tahoma_7b_green2Small.drawString(g, strTool[i], wScroll / 2, num2 + 6, mFont.CENTER);
    ...
    private void paintGameSubInfo(mGraphics g)
    {
        ...
        for (int i = 0; i < contenInfo.Length; i++)
        {
            ...
            if (num2 - cmy <= yScroll + hScroll && num2 - cmy >= yScroll - ITEM_HEIGHT)
            {
                mFont.tahoma_7b_green2Small.drawString(g, contenInfo[i], xScroll + 5, num2 + 6, mFont.LEFT);
            }
        }
        paintScrollArrow(g);
    }
    ...
    private void paintGameInfo(mGraphics g)
    {
        ...
        for (int i = 0; i < vGameInfo.size(); i++)
        {
            ...
            if (num2 - cmy <= yScroll + hScroll && num2 - cmy >= yScroll - ITEM_HEIGHT)
            {
                g.setColor((i != selected) ? 15196114 : 16383818);
                g.fillRect(num, num2, num3, h);
                mFont.tahoma_7b_green2Small.drawString(g, gameInfo.main, (int)(GameCanvas.w * 0.025), num2 + 6, mFont.LEFT);
    ...
    private void paintSkill(mGraphics g)
    {
        ...
        for (int i = 0; i < num + 6; i++)
        {
            ...
            if (skill != null)
            {
                mFont.tahoma_7b_blue.drawString(g, skillTemplate.name + "[" + skillTemplate.id + "]", num2 + 5, num3 + 3, 0);
            ...
            else
            {
                ...
                mFont.tahoma_7b_green.drawString(g, skillTemplate.name + "[" + skillTemplate.id + "]", num2 + 5, num3 + 3, 0);
        ...
        for (int i = 0; i < this.currentListLength; i++)
        {
            ...
            if (archivement != null)
            {
                if (!archivement.isFinish)
                {
                    ...
                    mFont.tahoma_7_green.drawString(g, archivement.money.ToString() + " " + archivement.Reward, num + num3 - 5, num2, mFont.RIGHT);
    ...
    private void paintCombine(mGraphics g)
    {
        ...
        int h = ITEM_HEIGHT - 1;
        bool flag = false;
        for (int i = 0; i < ItemCombine.listCombine.Count; i++)
        {
            try
            {
                if (vItemCombine.elementAt(i) != null)
                {
                    y += ITEM_HEIGHT;
                    continue;
                }
            ...
            var itemCombine = ItemCombine.listCombine[i];
            var array = mFont.tahoma_7_blue.splitFontArray("Chọn " + itemCombine.info, w - 50);
            if (array.Length > 1)
            {
                h += 9 * (array.Length - 1);
                flag = true;
            ...
            SmallImage.drawSmallImage(g, itemCombine.IconID, 5 + x + ITEM_HEIGHT / 2, y + h / 2, 0, 3);
            mFont.tahoma_7_green2.drawString(g, itemCombine.name, 12 + x + ITEM_HEIGHT + 5, y + 1, 0);
            int yclone = y;
            for (int z = 0; z < array.Length; z++)
            {
                mFont.tahoma_7_blue.drawString(g, array[z], 12 + x + ITEM_HEIGHT + 5, yclone + 11, mFont.LEFT);
                yclone += 9;
            }
            ...
            //g.fillRect(num5, num6, num7, num8);
            //var itemCombine = ItemCombine.listCombine[i];
            //var array = mFont.tahoma_7_blue.splitFontArray("Chọn " + itemCombine.info,wScroll-35);
            //SmallImage.drawSmallImage(g, itemCombine.IconID, num5 + num7 / 2, num6 + num8 / 2, 0, 3);
            //mFont.tahoma_7_green2.drawString(g, itemCombine.name, num + 5, num2 + 1, 0);

            //mFont.tahoma_7_blue.drawString(g, "Chọn " + itemCombine.info, num + 5, num2 + 11, mFont.LEFT);



        }
        int max = Math.Max(vItemCombine.size(), ItemCombine.listCombine.Count);
        int num11 = yScroll + max * ITEM_HEIGHT;
        int num10 = ITEM_HEIGHT - 1;
        if (flag)
        {
            num11 = y;
        }
        if ((max == selected && keyTouchCombine == 1) || (!GameCanvas.isTouch && max == selected))
        {
            g.drawImage(GameScr.imgLbtnFocus, xScroll + wScroll / 2, num11 + num10 / 2 + 1, StaticObj.VCENTER_HCENTER);
            mFont.tahoma_7b_green2.drawString(g, mResources.UPGRADE, xScroll + wScroll / 2, num11 + num10 / 2 - 4, mFont.CENTER);
        }
        else
        {
            g.drawImage(GameScr.imgLbtn, xScroll + wScroll / 2, num11 + num10 / 2 + 1, StaticObj.VCENTER_HCENTER);
            mFont.tahoma_7b_dark.drawString(g, mResources.UPGRADE, xScroll + wScroll / 2, num11 + num10 / 2 - 4, mFont.CENTER);
        }
        if (vItemCombine.size() == 0)
        {
            if (combineInfo != null)
            {
                ...
                {
                    mFont.tahoma_7b_dark.drawString(g, combineInfo[i], xScroll + wScroll / 2, (num11 + num10 / 2) + 20 + (10 * i), 2);
                }
            }
    ...
    public static bool CheckSort(int type, Item item)
    {
        if (type == 0)
        {
            return true;
        ...
        if (item != null)
        {
            ...
            if (type == 3)
            {
                return item.template.type <= 4;
            }
            if (type == 4)
            {
                return Item.CheckItemHoTro(item) || item.template.type == 29;
            }
            if (type == 5)
            {
            ...
            if (type == 11)
            {
                return item.template.type == 39;
            }
            if (type == 12)
            {
                return item.template.type == 37;
            }
            if (type == 13)
            {
                return item.template.type == 76;
            }
        }
        return false;
    }
    public static Item[] SortItem(int typeSort)
    {
        List<Item> items = new List<Item>();
        foreach (Item item in Char.myCharz().arrItemBag)
        {
            if (CheckSort(typeSort, item))
            { items.Add(item); }
        }
        Item[] items1 = new Item[items.Count];
    ...
    public static int SortItemBag;
    public static bool showSort;
    public static string[] strsort = { "Tất Cả", "Trang bị", "Cải Trang", "Ngọc Rồng", "Linh Thú", "Pet", "Phụ Kiện", "Sao Pha Lê", "Đá Nâng Cấp", "Item Hổ Trợ", "Linh Tinh", "Danh Hiệu", "Hào Quang", "Linh hồn" };

    public static Command cmdSort = new Command("Tất Cả", -1, null);

    public static Command[] cmdSortShow =
    {
        new Command(strsort[0], -1, null),
        new Command(strsort[1], -1, null),
        new Command(strsort[2], -1, null),
        new Command(strsort[3], -1, null),
        new Command(strsort[4], -1, null),
        new Command(strsort[5], -1, null),
        new Command(strsort[6], -1, null),
        new Command(strsort[7], -1, null),
        new Command(strsort[8], -1, null),
        new Command(strsort[9], -1, null),
        new Command(strsort[10], -1, null),
        new Command(strsort[11], -1, null),
        new Command(strsort[12], -1, null),
        new Command(strsort[13], -1, null)
    };

    public static sbyte FindItem(Item item)
    {
    ...
    private bool isSorted(List<Item> list)
    {
        for (int i = 0; i < list.Count - 1; i++)
        {
            ...
            // Nếu gặp item null, bỏ qua vì item null luôn ở cuối
            if (item1 == null || item2 == null) continue;

            // Kiểm tra xem item1 có lớn hơn item2 không
            if (item1.template.type > item2.template.type ||
                (item1.template.type == item2.template.type && item1.template.id > item2.template.id))
            {
                return false; // Nếu phát hiện không đúng thứ tự, trả về false
            }
        }
        return true; // Trả về true nếu toàn bộ danh sách đã được sắp xếp
    }

    private ComboBox comboPanel = new ComboBox(new Action[]
    {
        () => {updateAllListInven();},// all,
        () => {SortItemBag = 1; } ,//nr
        () => {SortItemBag = 2; } ,//ct
        () => {SortItemBag = 3; }, //tb
        () => {SortItemBag = 4; }, //bt
        () => {SortItemBag = 5; } //khac
    }, "Tất cả", "Ngọc rồng", "Cải trang", "Trang bị", "Bổ trợ", "Khác");

    static Scroll scrollNgang = new Scroll();


    private static void updateAllListInven()
    {
        SortItemBag = 0;
    }
    void paintInventory2(mGraphics g)
    {
        g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
        g.setClip(xScroll, yScroll + 20, wScroll, hScroll - 20);

    

        g.translate(-scrollOffset, 0); // Dịch chuyển canvas theo scrollOffset
        Item[] arrItemBag = SortItem(SortItemBag);
        for (int i = 0; i < arrItemBag.Length; i++)
        {
            int j = i;
            int x = xScroll + 2 + (j / 6) * 30; // Cột cách nhau 30 pixel
            int w = 28;
            int y = yScroll + 28 + (j % 6) * (ITEM_HEIGHT + 1); // 6 ô dọc
            int h = ITEM_HEIGHT - 1;
            g.setColor(6838847);
            g.fillRect(x, y, w, h);
            g.setColor(14735817);
            g.fillRect(x + 1, y + 1, w - 2, h - 2);
            g.setColor(13553330, 0.8f);
            g.fillRect(x + 2, y + 2, w - 4, h - 4);
            mFont.tahoma_7_green2._drawString(g, i.ToString(), x + 1, y - 3, mFont.LEFT, 6);
            if (i == selected)
            {
                g.setColor(7040779);
                g.fillRect(x, y, w, h);
            ...
            if (item != null)
            {
                if (item.isSelect && GameCanvas.panel.type == 12)
                {
                ...
                if (item.quantity > 1)
                {
                    mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(item.quantity), x + w, y + h - mFont.tahoma_7b_yellow.getHeight(), mFont.RIGHT, mFont.tahoma_7b_dark);
                }
                if (comboPanel.openBox)
                {
                    return;
                }
            }
        ...
        g.reset();
        comboSort?.paint(g);
        Console.WriteLine($"paintInventory2: xScroll={xScroll}, wScroll={wScroll}, arrItemBag.Length={arrItemBag.Length}");
    }
    private void paintInventory(mGraphics g)
    {
        paintInventory2(g);
        return;
        cmdSort.caption = cmdSortShow[SortItemBag].caption;
        cmdSort.x = this.Equals(GameCanvas.panel) ? W : GameCanvas.panel2.X - (cmdSort.w);
        cmdSort.y = (int)(GameCanvas.h * 0.165);
        g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
        if (!showSort)
        { cmdSort.paint(g); }
        if (cmdSort.isPointerPressInside() && GameCanvas.isPointerClick)
        {
            if (!showSort)
                GameCanvas.clearAllPointerEvent();
            showSort = true;
        ...
        {
            int x = W;
            int y = (int)(GameCanvas.h * 0.05);
            for (int i = 0; i < cmdSortShow.Length; i++)
            {
                cmdSortShow[i].isFocus = i == SortItemBag;
                cmdSortShow[i].x = x;
                cmdSortShow[i].y = y;
                y += cmdSortShow[i].h;
                y += 2;
                if (y > GameCanvas.h - cmdSortShow[i].h)
                {
                    x += cmdSortShow[i].w;
                    x += 5;
                    y = (int)(GameCanvas.h * 0.05);
                }
                cmdSortShow[i].paint(g);
                if (cmdSortShow[i].isPointerPressInside())
                {
                    GameCanvas.clearAllPointerEvent();
                    cmy = cmtoY = 0;
                    SortItemBag = i;
                    cmdSort.caption = cmdSortShow[i].caption;
            ...
            try
            {
                ...
                if (item != null)
                {
                    if (item.isHaveOption(34))
                        g.setColor((i != selected) ? color1[0] : color2[0]);
                    else if (item.isHaveOption(35))
                    {
                        g.setColor((i != selected) ? color1[1] : color2[1]);
                    }
                    else if (item.isHaveOption(36))
                    {
                        g.setColor((i != selected) ? color1[2] : color2[2]);
                    }
                }
                g.fillRect(num6, num7, num8, num9);
                mFont.tahoma_7b_red.drawString(g, ((i + 1).ToString()), num6, num7 + num9 / 2, mFont.LEFT);
                if (item != null && item.isSelect && GameCanvas.panel.type == 12)
                {
                    g.setColor((i != selected) ? 6047789 : 7040779);
                    g.fillRect(num6, num7, num8, num9);
                ...
                if (item.itemOption != null)
                {
                    for (int j = 0; j < item.itemOption.Length; j++)
                    {
                        if (item.itemOption[j] != null && item.itemOption[j].optionTemplate != null && item.itemOption[j].optionTemplate.id == 72)
                            text = " [+" + item.itemOption[j].param + "]";
                    }
                }
    ...
    static int checkClickCount = 0;
    private void paintBody(mGraphics g)
    {
        g.setColor(16711680);
        //  g.setClip(xScroll, yScroll, wScroll, hScroll);
        g.translate(0, -cmy);
        ...
        for (int i = 0; i < arrItemBody.Length; i++)
        {
            ...
            {
                x = xScroll;
                y = yScroll + (itemHeight * i);
            }
            else if (i < 10)
            {
                x = xScroll + (width * 5);
                y = yScroll + (itemHeight * (i - 5));
            }
            else
            {
                if (arrItemBody.Length % 2 != 0)
                {
                    x = ((xScroll + wScroll) / 2) - width / 2;
                    y = yScroll + (itemHeight * 5);
                }
                else
                {
                    x = 1 + ((xScroll + wScroll) / 2);
                    y = yScroll + (itemHeight * 5);
                }
                if (i != 10)
                {
            ...
            g.fillRect(x, y, width + 1, itemHeight);
            g.setColor((i == selected) ? 16383818 : 15196114);
            g.fillRect(x + 1, y - 1, width - 1, itemHeight - 1);
            g.setColor((i == selected) ? 9541120 : 9993045);
            g.drawRect(xScroll + width, yScroll, width * 4, (itemHeight * 5) - 1);
            var head = GameScr.parts[Char.myCharz().head];
            var body = GameScr.parts[Char.myCharz().body];
            var leg = GameScr.parts[Char.myCharz().leg];
            if (head != null && head.pi != null && body != null && body.pi != null && leg != null && leg.pi != null)
            {
                if (SmallImage.imgNew[head.id] == null || SmallImage.imgNew[body.id] == null || SmallImage.imgNew[leg.id] == null)
                {
                    SmallImage.createImage(head.id);
                    SmallImage.createImage(body.id);
                    SmallImage.createImage(leg.id);
                    return;
                }
                var imgHead = SmallImage.imgNew[head.id].img;
                var imgBody = SmallImage.imgNew[body.id].img;
                var imgLeg = SmallImage.imgNew[leg.id].img;
                ...
                int centerX = ((xScroll + wScroll) / 2) / mGraphics.zoomLevel * 2;
                int centerY = (yScroll + (itemHeight * 4)) / mGraphics.zoomLevel * 2;

                GUIUtility.ScaleAroundPivot(new Vector2(2f, 2f), new Vector2(centerX + maxWidth, centerY + height));
                g.drawImage(TileMap.bong, centerX, centerY, 3);
                Char.myCharz().paintCharBody(g, centerX, centerY, 1, Char.myCharz().cf, false);
                GUI.matrix = oldMatrix;
            }

            if (GameCanvas.isPointerHoldIn(x, y, width, itemHeight) && GameCanvas.isPointerClick)
            {
                selected = i;
                if (checkClick < 10)
                {
                    checkClick++; ;
                    checkClickCount = selected;
                }

            }
            if (checkClick >= 10 && !GameCanvas.isPointerDown)
            {
                pointerDownTime = 2;
                waitToPerform = 1;
                checkClick = -1;
            ...
            if (item == null)
            {
                ...
                {
                    int yClone = y;
                    string[] data = mFont.tahoma_7.splitFontArray(dataBody[i], 8, 5);
                    foreach (string s in data)
                    {
                        mFont.tahoma_7._drawString(g, s, x + (width / 2), yClone, mFont.CENTER, 5);
                        yClone += 5;
                    }
                }
                continue;
            }
            SmallImage.drawSmallImage(g, item.template.iconID, x + width / 2, y + itemHeight / 2, 0, 3);
            ...
            //        if (dataBody[i] != null)
            //        {
            //            mFont.tahoma_7b_green.drawString(g, dataBody[i], num3 + 5, num4 + 1, 0);
            //        }
            //    }
            //    catch { }
            //    continue;
    ...
    private void paintTab(mGraphics g)
    {
        ...
        if (currentTabIndex == 4 && mainTabName.Length != 5)
        {
            g.translate(-cmx, 0);
        }
        for (int i = 0; i < currentTabName.Length; i++)
    ...
    private void paintBottomMoneyInfo(mGraphics g)
    {
        if (type != 13 || (currentTabIndex != 2 && !Equals(GameCanvas.panel2)))
        {
            ...
            g.drawImage(imgLuong, X + 120, H - 8, 3);
            mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(Char.myCharz().xu), X + 24, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
            mFont.tahoma_7_yellow.drawString(g, Char.myCharz().luongStr + string.Empty, X + 130, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
        }
    }
    ...
    private void paintToolInfo(mGraphics g)
    {
        ...
        //mFont.tahoma_7_yellow._drawString(g, mResources.character + ": " + Char.myCharz().cName, 60, 16, mFont.LEFT, 10);
    ...
    public static string[] petinfo = new string[] { };
    private void paintPetInfo(mGraphics g)
    {
        //int y = 4;
        //for (int i = 0; i < petinfo.Length; i++)
        //{
        //    mFont.tahoma_7_yellow.drawString(g, petinfo[i], X + 60, y, mFont.LEFT, mFont.tahoma_7_grey);
        //    y += 11;
        //}
        ...
        {
            mFont.tahoma_7_yellow.drawString(g, mResources.hit_point + " :" + Char.myPetz().cDamFull, X + 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
        }
        mFont.tahoma_7_yellow.drawString(g, mResources.vitality + " :" + Char.myPetz().cMaxStamina, X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
        mFont.tahoma_7_yellow.drawString(g, "Chỉ số cộng thêm" + " : " + Panel.chisocongthem + "%", X + 60, 38, mFont.LEFT, mFont.tahoma_7_grey);
        g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
    }

    private void paintCharInfo(mGraphics g, Char c)
    {
        mFont.tahoma_7b_white.drawString(g, c.cName, X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
        c.paintTickBlue(g, X + 60 + (int)(mFont.tahoma_7b_white.getWidth(c.cName) * 1.5), 4 + (int)(mFont.tahoma_7b_white.getHeight() * 1.15));
        mFont.tahoma_7_yellow.drawString(g, mResources.vitality + " :" + c.cMaxStamina, X + 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
        g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
        if (c.cPower > 0)
        {
    ...
    private void paintPetStatusInfo(mGraphics g)
    {
        ...
        mFont.tahoma_7_yellow.drawString(g, "Điểm tiềm năng" + ": " + NinjaUtil.getMoneys(Char.myPetz().cTiemNang), X + 60, 38, mFont.LEFT, mFont.tahoma_7_grey);
    }
    ...
    private void doFireInventory()
    {
        ...
        currItem = null;
        MyVector myVector = new MyVector();
        Item item = SortItem(SortItemBag)[selected];
        if (item != null)
        {
            currItem = item;
            ...
            if (this.currItem.template.id != 457 && !currItem.isTypeBody())
            {
                ...
                else
                {
                    ...
                    Debug.Log("ID Auto :" + (int)this.currItem.template.id);
                }
            }
            if (this.currItem.template.type == 12 && currItem.template.id != 14 && currItem.quantity >= 7)
            {
                myVector.addElement(new Command("Hợp Ngọc Rồng", AutoItem.gI(), 11, currItem));
            }
        }
    ...
    private static void VMN()
    {
        MyVector myVector = new MyVector();
        myVector.addElement(new Command("Đóng", ZamasuMain.gI(), 1, null));
        GameCanvas.menu.startAt(myVector, 3);
        string str = string.Concat(new object[] {
        "\n|1|ak: Tự động đánh",
        "\nts : Tàn Sát Quái Siêu Tốk",
        "\ntdc X: Tốc độ chạy nhân vật",
        "\nhs: Auto hồi sinh",
        "\nspl: Info sao pha lê",
        "\ncd: Auto đậu cho đệ",
        "\ntbb: Thông báo boss",
        ...
        "\ns X: tốc độ X",
        "\nk X: Chuyển khu X",
        "\n\nPhím Tắt: A: Tự Đánh - C: Capsule - F: Bông Tai - E: Hồi Sinh" +
        "\n G: Giao Dịch - M: Chuyển Khu " +
        "\nJ,K,L: Chuyển Map Trái,Giữa,Phải" +
        "\n S : Auto Focus Boss,T : Tàn sát,N : Nhặt"});
        ChatPopup.addChatPopup(str, 100000, new Npc(-1, 1, -1, -1, -1, 7992));
    }
    ...
    private void setTypeGameSubInfo()
    {
        ...
        contenInfo = mFont.tahoma_7_grey.splitFontArray(content, wScroll - 10);
    ...
    private void doFireSkill()
    {
        ...
        if (selected >= 0 && selected <= 5)
        {
            ...
            if (selected == 0)
            {
                ...
                if (cTiemNang >= 100 * (2 * (cHPGoc + num2) + 1980) / 2 && cTiemNang < 1000 * (2 * (cHPGoc + num2) + 19800) / 2)
                {
                    MyVector myVector2 = new MyVector(string.Empty);
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num2), this, 9000, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num2) + 180) / 2), this, 9006, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(100 * Char.myCharz().hpFrom1000TiemNang) + mResources.HP + "\n-" + Res.formatNumber2(100 * (2 * (cHPGoc + num2) + 1980) / 2), this, 9007, null));
                    GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);

                }
                if (cTiemNang >= 1000 * (2 * (cHPGoc + num2) + 19800) / 2 && cTiemNang < 10000 * (2 * (cHPGoc + num2) + 199980) / 2)
                {
                    MyVector myVector2 = new MyVector(string.Empty);
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num2), this, 9000, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num2) + 180) / 2), this, 9006, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(100 * Char.myCharz().hpFrom1000TiemNang) + mResources.HP + "\n-" + Res.formatNumber2(100 * (2 * (cHPGoc + num2) + 1980) / 2), this, 9007, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(1000 * Char.myCharz().hpFrom1000TiemNang) + mResources.HP + "\n-" + Res.formatNumber2(1000 * (2 * (cHPGoc + num2) + 19980) / 2), this, 9008, null));
                    GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);

                }
                if (cTiemNang >= 10000 * (2 * (cHPGoc + num2) + 199980) / 2)
                {
                    MyVector myVector2 = new MyVector(string.Empty);
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num2), this, 9000, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num2) + 180) / 2), this, 9006, null));
                    ...
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(100 * Char.myCharz().hpFrom1000TiemNang) + mResources.HP + "\n-" + Res.formatNumber2(100 * (2 * (cHPGoc + num2) + 1980) / 2), this, 9007, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(1000 * Char.myCharz().hpFrom1000TiemNang) + mResources.HP + "\n-" + Res.formatNumber2(1000 * (2 * (cHPGoc + num2) + 19980) / 2), this, 9008, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(10000 * Char.myCharz().hpFrom1000TiemNang) + mResources.HP + "\n-" + Res.formatNumber2(10000 * (2 * (cHPGoc + num2) + 199980) / 2), this, 9009, null));
                    GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);
            ...
            if (selected == 1)
            {
                ...
                if (cTiemNang >= 100 * (2 * (cMPGoc + num2) + 1980) / 2 && cTiemNang < 1000 * (2 * (cMPGoc + num2) + 19800) / 2)
                {
                ...
                if (cTiemNang >= 1000 * (2 * (cMPGoc + num2) + 19800) / 2 && cTiemNang < 10000 * (2 * (cMPGoc + num2) + 199980) / 2)
                {
                    MyVector myVector2 = new MyVector(string.Empty);
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(cMPGoc + num2), this, 9000, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(10 * (2 * (cMPGoc + num2) + 180) / 2), this, 9006, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(100 * Char.myCharz().mpFrom1000TiemNang) + mResources.KI + "\n-" + Res.formatNumber2(100 * (2 * (cMPGoc + num2) + 1980) / 2), this, 9007, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(1000 * Char.myCharz().mpFrom1000TiemNang) + mResources.KI + "\n-" + Res.formatNumber2(1000 * (2 * (cMPGoc + num2) + 19980) / 2), this, 9008, null));
                    GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);

                }
                if (cTiemNang >= 10000 * (2 * (cMPGoc + num2) + 199980) / 2)
                {
                    MyVector myVector2 = new MyVector(string.Empty);
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(cMPGoc + num2), this, 9000, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(10 * (2 * (cMPGoc + num2) + 180) / 2), this, 9006, null));
                    ...
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(100 * Char.myCharz().mpFrom1000TiemNang) + mResources.KI + "\n-" + Res.formatNumber2(100 * (2 * (cMPGoc + num2) + 1980) / 2), this, 9007, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(1000 * Char.myCharz().mpFrom1000TiemNang) + mResources.KI + "\n-" + Res.formatNumber2(1000 * (2 * (cMPGoc + num2) + 19980) / 2), this, 9008, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + NinjaUtil.getMoneys(10000 * Char.myCharz().mpFrom1000TiemNang) + mResources.KI + "\n-" + Res.formatNumber2(10000 * (2 * (cMPGoc + num2) + 199980) / 2), this, 9009, null));
                    GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);
            ...
            if (selected == 2)
            {
                ...
                if (cTiemNang >= 100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd && cTiemNang < 1000 * (2 * cDamGoc + 999) / 2 * Char.myCharz().expForOneAdd)
                {
                ...
                if (cTiemNang >= 1000 * (2 * cDamGoc + 999) / 2 * Char.myCharz().expForOneAdd && cTiemNang < 10000 * (2 * cDamGoc + 9999) / 2 * Char.myCharz().expForOneAdd)
                {
                    MyVector myVector6 = new MyVector(string.Empty);
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(cDamGoc * 100), this, 9000, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd), this, 9006, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd), this, 9007, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 1000 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(1000 * (2 * cDamGoc + 999) / 2 * Char.myCharz().expForOneAdd), this, 9008, null));

                    GameCanvas.menu.startAt(myVector6, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);
                }
                if (cTiemNang >= 10000 * (2 * cDamGoc + 9999) / 2 * Char.myCharz().expForOneAdd)
                {
                    MyVector myVector6 = new MyVector(string.Empty);
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(cDamGoc * 100), this, 9000, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd), this, 9006, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd), this, 9007, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 1000 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(1000 * (2 * cDamGoc + 999) / 2 * Char.myCharz().expForOneAdd), this, 9008, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 10000 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(10000 * (2 * cDamGoc + 9999) / 2 * Char.myCharz().expForOneAdd), this, 9009, null));
            ...
            if (selected == 3)
            {
                ...
                if (Char.myCharz().cTiemNang >= number2)
                {
                    myVector7.addElement(new Command(mResources.use_potential_point_for1 + "\n10 " + mResources.armor + "\n" + Res.formatNumber2(number2), this, 9006, null));
                    if (Char.myCharz().cTiemNang >= number3)
    ...
    private void doFireBox()
    {
        ...
        if (!flag)
        {
            ...
            bool flag6 = this.currentTabIndex == 1 || this.Equals(GameCanvas.panel2);
            if (flag6)
            {

                Item item2 = global::Char.myCharz().arrItemBag[(int)selected];
                bool flag8 = item2 != null;
                if (flag8)
                {
                    myVector.addElement(new Command(mResources.move_to_chest, this, 1001, item2));
                    bool flag9 = item2.isTypeBody();
                    if (flag9)
                    {
            ...
            bool flag11 = this.currItem != null;
            if (flag11)
            {
    ...
    public void perform(int idAction, object p)
    {
        if (idAction == 19000)
        {
            Service.gI().upPotentialPet(selected, 1);
            GameCanvas.endDlg();
            InfoDlg.showWait();
        }
        if (idAction == 19006)
        {
            Service.gI().upPotentialPet(selected, 10);
            GameCanvas.endDlg();
            InfoDlg.showWait();
        }
        if (idAction == 19007)
        {
            Service.gI().upPotentialPet(selected, 100);
            GameCanvas.endDlg();
            InfoDlg.showWait();
        }
        if (idAction == 15000)
        {
            int num = (int)p;
            Service.gI().upGradeSkillPet(num - 5);
            Debug.Log(num - 5);
        }
        if (idAction == 9999)
        {
        ...
        {
            Item item2 = (Item)p;
            for (int j = 0; j < GameCanvas.panel.vItemCombine.size(); j++)
            {
                Item item5 = (Item)GameCanvas.panel.vItemCombine.elementAt(j);
                if (item5.indexUI == item2.indexUI)
                {
        ...
        if (idAction == 1001)
        {
            ...
            Service.gI().getItem(BAG_BOX, (sbyte)selected);
        }
        if (idAction == 1003)
        {
            hide();
        ...
        if (idAction == 2000)
        {
            Service.gI().getItem(BAG_BODY, FindItem(currItem));
        }
        if (idAction == 2001)
        {
            Res.outz("use item");
            ...
            Service.gI().useItem(0, (sbyte)((!inventorySelect_isbody) ? 1 : 0), FindItem(currItem), -1);
        ...
        if (idAction == 2003)
        {
            if (currentTabIndex == 1)
            {
                Service.gI().useItem(1, 0, FindItem(currItem), -1);
            }
            if (currentTabIndex == 2)
            {
                Service.gI().useItem(1, 1, FindItem(currItem), -1);
            }
        }
        if (idAction == 2004)
        {
        ...
        if (idAction == 2005)
        {
            ...
            Service.gI().getItem(BAG_PET, FindItem(currItem));
        }
        if (idAction == 2006)
        {
        ...
        if (idAction == 3002)
        {
            Service.gI().saleItem(0, (sbyte)(1), FindItem(currItem));
        }
        if (idAction == 3003)
        {
            GameCanvas.endDlg();
        ...
        if (idAction == 9008)
        {
            Service.gI().upPotential(selected, 1000);
            GameCanvas.endDlg();
            InfoDlg.showWait();
        }
        if (idAction == 9009)
        {
            Service.gI().upPotential(selected, 10000);
            GameCanvas.endDlg();
            InfoDlg.showWait();
        }
        if (idAction == 9002)
    ...
    private void paintOption(mGraphics g)
    {
        ...
        for (int i = 0; i < strCauhinh.Length; i++)
        {
            ...
            if (num - cmy <= yScroll + hScroll && num - cmy >= yScroll - ITEM_HEIGHT)
            {
                g.setColor((i != selected) ? 15196114 : 16383818);
                g.fillRect(x, num, num2, h);
                mFont.tahoma_7b_green2Small.drawString(g, strCauhinh[i], xScroll + 10, num + 6, mFont.LEFT);
            }
        }
        paintScrollArrow(g);
    }
    ...
    {
        if ((type == 0 && currentTabIndex == 2) || (type == 7 && currentTabIndex == 0))
        {
            return true;
        }
        return false;
    ...
    private void setTabPetSkill2()
    {
        ITEM_HEIGHT = 30;
        Skill[] arrPetSkill = Char.myPetz().arrPetSkill;
        currentListLength = arrPetSkill.Length + 6;
        cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
        cmy = (cmtoY = cmyLast[currentTabIndex]);
        if (cmyLim < 0)
        {
    ...
    private void paintPetSkill(mGraphics g)
    {
        ...
        g.setClip(xScroll, yScroll, wScroll, hScroll);
        g.translate(0, -cmy);
        int num = 6 + Char.myPetz().arrPetSkill.Length;
        for (int i = 0; i < num; i++)
        {
            int num2 = xScroll + 30;
            int num3 = yScroll + i * 30;
            int num4 = wScroll - 30;
            int h = 29;
            int num5 = xScroll;
            int num6 = (yScroll + i * 30) + 1;
            if (num3 - cmy > yScroll + hScroll || num3 - cmy < yScroll - ITEM_HEIGHT)
                continue;
            g.setColor((i != selected) ? 15196114 : 16383818);
            g.fillRect(num2, num3, num4, h);
            ...
            if (i == 0)
            {
                SmallImage.drawSmallImage(g, 567, num5, num6, 0, 0);
                mFont.tahoma_7b_blue.drawStringBorder(g, "HP: " + NinjaUtil.getMoneys(Char.myPetz().cHPGoc), num2 + 3, num3, 0);
                mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(Char.myPetz().cHPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myPetz().hpFrom1000TiemNang, num2 + 5, num3 + 12, 0);
            }
            if (i == 1)
            {
                SmallImage.drawSmallImage(g, 569, num5, num6, 0, 0);
                string st2 = "KI: " + NinjaUtil.getMoneys(Char.myPetz().cMPGoc);
                mFont.tahoma_7b_blue.drawString(g, st2, num2 + 3, num3, 0);
                mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(Char.myPetz().cMPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myPetz().mpFrom1000TiemNang, num2 + 5, num3 + 12, 0);
            }
            if (i == 2)
            {
                SmallImage.drawSmallImage(g, 568, num5, num6, 0, 0);
                string st3 = "Sức Đánh: " + NinjaUtil.getMoneys(Char.myPetz().cDamGoc);
                mFont.tahoma_7b_blue.drawString(g, st3, num2 + 3, num3, 0);
                mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(Char.myPetz().cDamGoc * 100) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myPetz().damFrom1000TiemNang, num2 + 5, num3 + 15, 0);
            }
            if (i == 3)
            {
                SmallImage.drawSmallImage(g, 721, num5, num6, 0, 0);
                string st4 = "Giáp: " + NinjaUtil.getMoneys(Char.myPetz().cDefGoc);
                mFont.tahoma_7b_blue.drawString(g, st4, num2 + 3, num3, 0);
                mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(500000 + Char.myPetz().cDefGoc * 100000) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myPetz().defFrom1000TiemNang, num2 + 5, num3 + 15, 0);
            }
            if (i == 4)
            {
                SmallImage.drawSmallImage(g, 719, num5, num6, 0, 0);
                string st5 = mResources.critical + " " + mResources.root + ": " + Char.myPetz().cCriticalGoc + "%";
                long num9 = 50000000L;
                int num10 = Char.myPetz().cCriticalGoc;
                if (num10 > t_tiemnang.Length - 1)
                {
                    num10 = t_tiemnang.Length - 1;
                }
                ...
                mFont.tahoma_7b_blue.drawString(g, st5, num2 + 5, num3 + 3, 0);
                long number = num9;
                mFont.tahoma_7_green2.drawString(g, Res.formatNumber2(number) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myPetz().criticalFrom1000Tiemnang, num2 + 5, num3 + 15, 0);

            }
            if (i == 5)
            {
                if (specialInfopet != null)
                {
                    SmallImage.drawSmallImage(g, spearcialImagepet, num5 - 1, num6, 0, 0);
                    string[] array = mFont.tahoma_7.splitFontArray(specialInfopet, 120);
                    for (int j = 0; j < array.Length; j++)
                    {
                        mFont.tahoma_7_green2.drawString(g, array[j], num2 + 5, num3 + 3 + j * 12, 0);
                    }
            ...
            if (i >= 6)
            {
                if (Char.myPetz().arrPetSkill[i - 6].template == null)
                {
                    SmallImage.drawSmallImage(g, 6123, num5, num6, 0, 0);
                    mFont.tahoma_7b_blue.drawString(g, "Chưa Có", num2 + 3, num3, 0);

                }
                else
                {
                    SkillTemplate skillTemplate = Char.myPetz().arrPetSkill[i - 6].template;
                    Skill skill = Char.myPetz().arrPetSkill[i - 6];
                    SmallImage.drawSmallImage(g, skillTemplate.iconId, num5, num6, 0, 0);
                    mFont.tahoma_7_blue.drawString(g, skillTemplate.name, num2 + 3, num3, 0);
                    mFont.tahoma_7_green2.drawString(g, mResources.level + ": " + skill.point, num2 + 3, num3 + 15, 0);
                }
            }
        }
        paintScrollArrow(g);
    ...
    private void doFirePetSkill()
    {
        ...
        if (selected == 0 || selected == 1 || selected == 2 || selected == 3 || selected == 4)
        {
            ...
            if (selected == 0)
            {
                if (cTiemNang < Char.myPetz().cHPGoc + num2)
                {
                    GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Char.myPetz().cTiemNang + mResources.not_enough_potential_point2 + (Char.myPetz().cHPGoc + num2));
                    return;
                }
                if (cTiemNang > cHPGoc && cTiemNang < 10 * (2 * (cHPGoc + num2) + 180) / 2)
                {
                    GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + (cHPGoc + num2) + mResources.use_potential_point_for2 + Char.myPetz().hpFrom1000TiemNang + mResources.for_HP, new Command(mResources.increase_upper, this, 19000, null), new Command(mResources.CANCEL, this, 4007, null));
                    return;
                }
                if (cTiemNang >= 10 * (2 * (cHPGoc + num2) + 180) / 2 && cTiemNang < 100 * (2 * (cHPGoc + num2) + 1980) / 2)
                {
                    MyVector myVector = new MyVector(string.Empty);
                    myVector.addElement(new Command(mResources.increase_upper + "\n" + Char.myPetz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num2), this, 19000, null));
                    myVector.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myPetz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num2) + 180) / 2), this, 19006, null));
                    GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected, Char.myPetz());
                }
                if (cTiemNang >= 100 * (2 * (cHPGoc + num2) + 1980) / 2)
                {
                    ...
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + Char.myPetz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num2), this, 19000, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myPetz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num2) + 180) / 2), this, 19006, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myPetz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(100 * (2 * (cHPGoc + num2) + 1980) / 2), this, 19007, null));
                    GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected, Char.myPetz());
                }
            }
            if (selected == 1)
            {
                if (Char.myPetz().cTiemNang < Char.myPetz().cMPGoc + num2)
                {
                    GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Char.myPetz().cTiemNang + mResources.not_enough_potential_point2 + (Char.myPetz().cMPGoc + num2));
                    return;
                }
                if (cTiemNang > cMPGoc && cTiemNang < 10 * (2 * (cMPGoc + num2) + 180) / 2)
                {
                    GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + (cMPGoc + num2) + mResources.use_potential_point_for2 + Char.myPetz().mpFrom1000TiemNang + mResources.for_KI, new Command(mResources.increase_upper, this, 19000, null), new Command(mResources.CANCEL, this, 4007, null));
                    return;
                }
                if (cTiemNang >= 10 * (2 * (cMPGoc + num2) + 180) / 2 && cTiemNang < 100 * (2 * (cMPGoc + num2) + 1980) / 2)
                {
                    MyVector myVector3 = new MyVector(string.Empty);
                    myVector3.addElement(new Command(mResources.increase_upper + "\n" + Char.myPetz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(cHPGoc + num2), this, 19000, null));
                    myVector3.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myPetz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num2) + 180) / 2), this, 19006, null));
                    GameCanvas.menu.startAt(myVector3, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected, Char.myPetz());
                }
                ...
                {
                    MyVector myVector4 = new MyVector(string.Empty);
                    myVector4.addElement(new Command(mResources.increase_upper + "\n" + Char.myPetz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(cMPGoc + num2), this, 19000, null));
                    myVector4.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myPetz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(10 * (2 * (cMPGoc + num2) + 180) / 2), this, 19006, null));
                    myVector4.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myPetz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(100 * (2 * (cMPGoc + num2) + 1980) / 2), this, 19007, null));
                    GameCanvas.menu.startAt(myVector4, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected, Char.myPetz());
                }
            }
            ...
            {
                if (cTiemNang < cDamGoc * Char.myPetz().expForOneAdd)
                {
                    GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Char.myPetz().cTiemNang + mResources.not_enough_potential_point2 + cDamGoc * 100);
                    return;
                }
                if (cTiemNang > cDamGoc && cTiemNang < 10 * (2 * cDamGoc + 9) / 2 * Char.myPetz().expForOneAdd)
                {
                    GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + cDamGoc * 100 + mResources.use_potential_point_for2 + Char.myPetz().damFrom1000TiemNang + mResources.for_hit_point, new Command(mResources.increase_upper, this, 19000, null), new Command(mResources.CANCEL, this, 4007, null));
                    return;
                }
                if (cTiemNang >= 10 * (2 * cDamGoc + 9) / 2 * Char.myPetz().expForOneAdd && cTiemNang < 100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd)
                {
                    MyVector myVector5 = new MyVector(string.Empty);
                    myVector5.addElement(new Command(mResources.increase_upper + "\n" + Char.myPetz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(cDamGoc * 100), this, 19000, null));
                    myVector5.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myPetz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd), this, 19006, null));
                    addSkillDetail2(selected, Char.myPetz());
                }
                if (cTiemNang >= 100 * (2 * cDamGoc + 99) / 2 * Char.myPetz().expForOneAdd)
                {
                    MyVector myVector6 = new MyVector(string.Empty);
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + Char.myPetz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(cDamGoc * 100), this, 19000, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myPetz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd), this, 19006, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myPetz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd), this, 19007, null));
                    GameCanvas.menu.startAt(myVector6, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected, Char.myPetz());
                }
            }
            ...
            {
                if (Char.myPetz().cTiemNang < 50000 + Char.myPetz().cDefGoc * 1000)
                {
                    GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + NinjaUtil.getMoneys(Char.myPetz().cTiemNang) + mResources.not_enough_potential_point2 + NinjaUtil.getMoneys(50000 + Char.myPetz().cDefGoc * 1000));
                    return;
                }
                long number = (long)(2 * (cDefGoc + 5)) / 2L * 100000;
                long number2 = 10L * (long)(2 * (cDefGoc + 5) + 9) / 2 * 100000;
                ...
                mResources.use_potential_point_for1 = mResources.increase_upper;
                MyVector myVector7 = new MyVector(string.Empty);
                myVector7.addElement(new Command(mResources.use_potential_point_for1 + "\n1 " + mResources.armor + "\n" + Res.formatNumber2(number), this, 19000, null));
                myVector7.addElement(new Command(mResources.use_potential_point_for1 + "\n10 " + mResources.armor + "\n" + Res.formatNumber2(number2), this, 19006, null));
                myVector7.addElement(new Command(mResources.use_potential_point_for1 + "\n100 " + mResources.armor + "\n" + Res.formatNumber2(number3), this, 19007, null));
                GameCanvas.menu.startAt(myVector7, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                addSkillDetail2(selected, Char.myPetz());
            }
            else if (selected == 4)
            {
                long num3 = 50000000L;
                int num4 = Char.myPetz().cCriticalGoc;
                if (num4 > t_tiemnang.Length - 1)
                {
                    num4 = t_tiemnang.Length - 1;
                }
                num3 = t_tiemnang[num4];
                if (Char.myPetz().cTiemNang < num3)
                {
                    GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Res.formatNumber2(Char.myPetz().cTiemNang) + mResources.not_enough_potential_point2 + Res.formatNumber2(num3));
                    return;
                }
                GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + Res.formatNumber(num3) + mResources.use_potential_point_for2 + Char.myPetz().criticalFrom1000Tiemnang + mResources.for_crit, new Command(mResources.increase_upper, this, 19000, null), new Command(mResources.CANCEL, this, 4007, null));
            }
        }
        else if (selected == 5)
        {
            Service.gI().speacialSkill(1);
        }
        else if (selected >= 6)
        {
            Skill skill = Char.myPetz().arrPetSkill[selected - 6];
            if (skill != null && skill.point < 7)
            {
                MyVector myVector7 = new MyVector(string.Empty);
                myVector7.addElement(new Command("Nâng cấp \n kỹ năng \n " + getGemUpgradeSkill(skill, selected - 6) + " ngọc", this, 15000, selected));
                GameCanvas.menu.startAt(myVector7, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                return;
            }
        }
    }
    public static int getGemUpgradeSkill(Skill skill, int level)
    {
        int nextgem = -1;
        switch (level)
        {
            case 0: nextgem = 100; break;
            case 1: nextgem = 150; break;
            case 2: nextgem = 200; break;
            case 3: nextgem = 500; break;
            case 4: nextgem = 1000; break;
            case 5: nextgem = 1000; break;
        }
        return nextgem;
    }


    private void addSkillDetail2(int type, Char @char)
    {
        string empty = string.Empty;
        long num = 0;
        if (selected == 0)
        ...
        if (selected == 2)
        {
            num = @char.cDamGoc * Char.myCharz().expForOneAdd;
        }
        if (selected == 3)
        {
    ...
    private void paintPetSkillInfo(mGraphics g)
    {
        mFont.tahoma_7b_white.drawString(g, "HP: " + Char.myPetz().cHP + "/" + Char.myPetz().cHPFull, X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
        mFont.tahoma_7b_white.drawString(g, "MP: " + Char.myPetz().cMP + "/" + Char.myPetz().cMPFull, X + 60, 16, mFont.LEFT, mFont.tahoma_7b_dark);
        mFont.tahoma_7_yellow.drawString(g, mResources.critical + ": " + Char.myPetz().cCriticalFull + "   " + mResources.armor + ": " + Char.myPetz().cDefull, X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
