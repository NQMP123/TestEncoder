...
public class ServerListScreen : mScreen, IActionListener
{
    ...
    public static string TestIP = "Test:221.132.33.169:14445:0,0,0";
    public static string WebIP	 = "Thần thoại 01:160.191.2.73:14445:0,0,0";
    public static string WebIP2 = "Test:sv1.thanthoainro.com:14445:0,0,0";
    public static string mainIP = Decrypt("➇❙❓❓➂❁✿❶❠❁❈➄❘❓❐➇❞❽❝❁❜❢❄➄❲❼❈❺❱❁❀❺❱❁✿➄❨❁❈➃❞❹❔✿❝❓❠❀❞❹❐➂❜❒➆➆", 9999);

    public static string smartPhoneVN = WebIP;
	...
	public static string linkweb = "https://thanthoainro.com";
    ...
    public static void paintBG(mGraphics g)
    {
        ...
        imgmove = Main.isPC ? imgmove + 1 : imgmove + 3;
    ...
    public override void paint(mGraphics g)
	{
		...
		if (GameCanvas.currentDialog == null)
		{
			if (!loadScreen)
			{
				if (!bigOk)
				{
					GameCanvas.gI().paintLogo(g, GameCanvas.hw, GameCanvas.hh - 32);
					if (!isGetData)
					{
					...
					else
					{
                        ...
                        GameCanvas.gI().paintLoading(g, false);
                        string download = percent + "%";
                        mFont.tahoma_7b_dark.drawString(g, download, GameCanvas.w / 2 + 2, GameCanvas.h / 2 - mFont.tahoma_7b_dark.getHeight() / 2, mGraphics.VCENTER | mGraphics.HCENTER);
                        mFont.tahoma_7b_dark.drawString(g, "Đang Tải Dữ Liệu", GameCanvas.w / 2 + 2, GameCanvas.hh + 35, mGraphics.VCENTER | mGraphics.HCENTER);
						string text = " . ";
						for (int i = 0; i < percent / 10; i++)
						{
							text += " . ";
						}
                        mFont.tahoma_7b_dark.drawString(g, text, GameCanvas.w / 2 + 2, GameCanvas.hh + 45, mGraphics.VCENTER | mGraphics.HCENTER);
					}
				}
			}
			else
			{
				...
				{
					GameCanvas.gI().paintLogo(g, GameCanvas.hw, (int)(GameCanvas.h*0.175));
				}
				for (int i = 0; i < cmd.Length; i++)
				{
					cmd[i].paint(g);
	...
	public override void updateKey()
	{
		if (GameCanvas.isTouch)
		{
            ...
            if (QLTKPanel.qltkCmd.isPointerPressInside())
                QLTKPanel.qltkCmd.performAction();
        }
		else if (loadScreen)
		{
