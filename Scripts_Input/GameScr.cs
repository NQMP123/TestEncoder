...
public class GameScr : mScreen, IChatable
{
    ...
    public void readPart()
    {
        ...
        try
        {
            ...
            datalog.Append("Num parts: ").Append(num).Append("\n");

            for (int i = 0; i < num; i++)
            {
                //parts[i].id = dataInputStream.readInt();
                int type = dataInputStream.readByte();
                parts[i] = new Part(type);
                datalog.Append("Part ").Append(i).Append(" - Type: ").Append(type).Append("\n");
                ...
                for (int j = 0; j < parts[i].pi.Length; j++)
                {
                    ...
                    datalog.Append("Part ").Append(i).Append(" - Image ").Append(j)
                           .Append(": ID = ").Append(parts[i].pi[j].id)
                           .Append(", DX = ").Append(parts[i].pi[j].dx)
                           .Append(", DY = ").Append(parts[i].pi[j].dy).Append("\n");
                }
            }
        ...
        if (Char.myCharz().mobFocus != null)
        {
            ...
            if (num <= Char.myCharz().myskill.dx && num2 <= Char.myCharz().myskill.dy)
            {
                ...
                if (num <= num3 && !flag2)
                {
                    ...
                    if (Char.myCharz().cx > Char.myCharz().mobFocus.getX())
                    {
                        Char.myCharz().cx = Char.myCharz().mobFocus.getX() + num3 + (flag ? 30 : 0);
                        Char.myCharz().cdir = -1;
                    }
                    else
                    {
            ...
            if (Char.myCharz().isMeCanAttackOtherPlayer(Char.myCharz().charFocus) || Char.myCharz().isSelectingSkillBuffToPlayer())
            {
                ...
                if (num10 <= Char.myCharz().myskill.dx && num11 <= Char.myCharz().myskill.dy)
                {
                    ...
                    if (num10 <= num12 && !flag4)
                    {
                        if (Char.myCharz().cx > Char.myCharz().charFocus.cx)
                        {
                            Char.myCharz().cx = Char.myCharz().charFocus.cx + num12;
                            Char.myCharz().cdir = -1;
                        }
                        else
                        {
        ...
        else
        {
            ...
            if (GameCanvas.keyAsciiPress != 0)
            {
                if (mobCapcha == null)
                {
                    if (TField.isQwerty)
                    {
                        Debug.Log(GameCanvas.keyPressed[1]);
    ...
    private void checkDrag()
    {
        ...
        if (isPointerDowning && !ViewBossbyNQMP.isShow)
        {
    ...
    private bool checkSingleClickEarly()
    {
        ...
        if (mapObject != null)
        {
            ...
            if ((Char.myCharz().mobFocus == mapObject || Char.myCharz().itemFocus == mapObject) && !Main.isPC)
            {
    ...
    {
        if (gamePad.disableClickMove() || TamBao.isShow)
        {
            return;
        }
        Char.myCharz().cancelAttack();
    ...
    public void autoPlay()
    {
        ...
        if (Char.myCharz().statusMe == 14 || Char.myCharz().statusMe == 5 || Char.myCharz().isCharge || Char.myCharz().isFlyAndCharge || Char.myCharz().isUseChargeSkill())
        {
            return;
        }
        bool flag = false;
        ...
        if (Char.myCharz().mobFocus.hp <= 0 || Char.myCharz().mobFocus.status == 1 || Char.myCharz().mobFocus.status == 0)
        {
            Char.myCharz().mobFocus = null;
        }
    ...
    public void doSelectSkill(Skill skill, bool isShortcut)
    {
        ...
        {
            Debug.Log(">>>use skill spec: " + skill.template.id);
    ...
    private void updateKeyTouchControl()
    {
        ...
        else if (isHaveSelectSkill)
        {
            ...
            {
                if (GameCanvas.isPointerHoldIn(15, 7, 60, 35) && GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                {
    ...
    public static bool checkJoinGame = true;
    public override void update()
    {
        if (GameCanvas.isPlaySound && mSystem.currentTimeMillis() - Service.lastSound >= 5000)
        {
            GameCanvas.isPlaySound = Rms.loadRmsInt("isPlaySound") == 1;
        }
        Main.screenShot.UpdateScreen();
        aaMod.Update();
        //if (checkJoinGame)
        //{
        //    Sound.playSoundJoinGame();
        //    checkJoinGame = false;
        //}
     ...
     //   SmallImage.update();
        try
        {
            ...
            {
                int num = (int)(dHP - Char.myCharz().cHP >> 1);
                if (num < 1)
                {
                    num = 1;
                }
    ...
    {
        if (ZamasuMain.gdh || GameCanvas.lowGraphic)
        {
            return;
        }
    ...
    public override void paint(mGraphics g)
    {
        try
        {
            ...
            if (!GameCanvas.lowGraphic && !ZamasuMain.gdh)
            {
            ...
            resetTranslate(g);
            paint_xp_bar(g);
            HairMod.ZamasuMain.Paint(g);
            HairMod.ClickHandler.paint(g);
            aaMod.Paint(g);
            ZamasuMain.PaintTT(g);
            ...
            if (!isPaintOther)
            {
                ...
                for (int i = 0; i < textTime.size(); i++)
                {
                    ((ItemTime)textTime.elementAt(i)).paintText(g, cmdMenu.x + ((Char.vItemTime.size() == 0) ? 25 : 5), ((Char.vItemTime.size() == 0) ? 60 : 90) + i * 12);
                }
                paintXoSo(g);
    ...
    public void paintImageBarRight(mGraphics g, Char c)
    {
        ...
        g.setClip(GameCanvas.w / 2 + 60 - 83 - hpBarW + hpBarW - num3, 5, num3, 10);
        g.drawImage(imgHPLost, GameCanvas.w / 2 + 60 - 83, 5, mGraphics.RIGHT | mGraphics.TOP);
        g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
        g.setClip(GameCanvas.w / 2 + 60 - 83 - hpBarW + hpBarW - num, 5, num, 10);
        g.drawImage(imgHP, GameCanvas.w / 2 + 60 - 83, 5, mGraphics.RIGHT | mGraphics.TOP);
        g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
        g.setClip(GameCanvas.w / 2 + 60 - 83 - mpBarW + hpBarW - num4, 20, num4, 6);
        g.drawImage(imgMPLost, GameCanvas.w / 2 + 60 - 83, 20, mGraphics.RIGHT | mGraphics.TOP);
        g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
        g.setClip(GameCanvas.w / 2 + 60 - 83 - mpBarW + hpBarW - num2, 20, num2, 6);
    ...
    private void paintImageBar(mGraphics g, bool isLeft, Char c)
    {
        if (c != null)
        {
            ...
            else
            {
                ...
                num2 = (int)(c.perCentMp * mpBarW / 100);
                num3 = (int)(c.cHP * hpBarW / c.cHPFull);
        ...
        else
        {
            ...
            else if (Char.myCharz().statusMe != 14)
            {
                ...
                else
                {
                    ...
                    g.setColor(16777215);
                    g.fillRect(xHP + 10, yHP + 10 + ((num3 != 0) ? (20 - num3) : 0) - 6, 20, (num3 == 0) ? 18 : num3);
    ...
    public void openUIZone(Message message)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < zones.Length; i++)
            {
                ...
                numPlayer[i] = message.reader().readShort();
    ...
    public void chatVip(string chatVip)
    {
        ...
        {
            aaMod.bossVip.addElement(new aaMod.BossFunctions(chatVip));
            if (aaMod.bossVip.size() > 5)
            {
                aaMod.bossVip.removeElementAt(0);
            }
        }
    ...
    public void paintChatVip(mGraphics g)
    {
        if (vChatVip.size() != 0 && isPaintChatVip)
        {
            ...
            mFont.tahoma_7_yellow.drawString(g, st, xChatVip, GameCanvas.h - 15, 0);
        }
    }
    ...
    {
        Effect_End eff = new Effect_End(type, subtype, typePaint, charUse, target, levelPaint, timeRemove, range);
        addEffect2Vector(eff);
    }
