...
public class mGraphics
{
    ...
    {
        Radius *= zoomLevel;
        float num = (float)(x * zoomLevel);
        float num2 = (float)(y * zoomLevel);
        w *= zoomLevel;
        h *= zoomLevel;
        ...
        if (this.isClip)
        {
            ...
            GUI.BeginGroup(new Rect((float)num4, (float)num5, (float)num6, (float)num7));
        }

        GUI.DrawTexture(new Rect(num - (float)num4, num2 - (float)num5, w, h), texture2D,
            ScaleMode.StretchToFill, false, 0, Color.white, new Vector4(boderSize, boderSize, boderSize, boderSize),
    ...
    public string getRichText(List<string> strs, List<string> colors)
    {
        if (strs.Count != colors.Count)
        {
            Debug.LogError("The number of strings must match the number of colors.");
            return "";
        }
        ...
        {
            richText += $"<color={colors[i]}>{strs[i]}</color>";
        }

        return richText;
    }

    public void drawRichText(List<string>strs, List<string> colors, int x, int y, int align)
    {
        if (strs.Count != colors.Count)
        {
            Debug.LogError("The number of strings must match the number of colors.");
        ...
        GUIStyle style = new GUIStyle(GUI.skin.label)
        {
            ...
            alignment = align == mFont.LEFT ? TextAnchor.UpperLeft : align == mFont.CENTER ? TextAnchor.UpperCenter : TextAnchor.UpperRight,
            richText = true
        };
        this.drawString(richText, x, y, style);
    }

    public void drawRichText(string[] strs, string[] colors, int x, int y, int align)
	{
        ...
        GUIStyle style = new GUIStyle(GUI.skin.name)
		{
			fontSize = 7 * mGraphics.zoomLevel,
			fontStyle = FontStyle.Bold,
            alignment = align == mFont.LEFT ? TextAnchor.UpperLeft : align == mFont.CENTER ? TextAnchor.UpperCenter : TextAnchor.UpperRight,
    ...
    public void fillTriangle(int x1, int y1, int x2, int y2, int x3, int y3)
    {
        ...
        // Tạo texture cho tam giác
        string key = "tr" + r + g + b + a;
        Texture2D texture2D = (Texture2D)cachedTextures[key];
        if (texture2D == null)
        {
            texture2D = new Texture2D(1, 1);
        ...
        // Chuẩn bị vẽ tam giác bằng GL
        MyVector triangleVector = new MyVector();
        triangleVector.addElement(new mLine(x1, y1, x2, y2, getIntByColor(new Color(r, g, b, a))));
        triangleVector.addElement(new mLine(x2, y2, x3, y3, getIntByColor(new Color(r, g, b, a))));
        triangleVector.addElement(new mLine(x3, y3, x1, y1, getIntByColor(new Color(r, g, b, a))));
        ...
        if (isClip)
        {
            GL.Vertex3(Mathf.Clamp(x1, clipX1, clipX1 + clipW1), Mathf.Clamp(y1, clipY1, clipY1 + clipH1), 0);
            GL.Vertex3(Mathf.Clamp(x2, clipX1, clipX1 + clipW1), Mathf.Clamp(y2, clipY1, clipY1 + clipH1), 0);
            GL.Vertex3(Mathf.Clamp(x3, clipX1, clipX1 + clipW1), Mathf.Clamp(y3, clipY1, clipY1 + clipH1), 0);
        }
        else
        {
            GL.Vertex3(x1, y1, 0);
