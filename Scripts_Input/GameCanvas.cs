...
public class GameCanvas : IActionListener
{
                ...
                else if (panel.isShow)
                {
                    ...
                    if (isPointer(panel.X + panel.W, panel.Y, w - panel.W * 2, panel.H) && isPointerJustRelease && panel.isDoneCombine)
                    {
                        ...
                        if (Panel.showSort)
                        {
                            ...
                            for (int i = 0; i < Panel.cmdSortShow.Length; i++)
                            {
                                if (Panel.cmdSortShow[i].isPointerPressInside())
                                {
                                    flag = true;
                                }
                            }
    ...
    public void paintLogo(mGraphics g,int x , int y)
    {
        ...
        {
            LoginScr.imgTitle = GameCanvas.loadImage("/mainImage/logo.png");
            return;
        }
        try
        {
            int y2 = (int)(GameCanvas.gameTick / ( (int)UnityEngine.Time.timeScale) % 10 * (mGraphics.getImageHeight(LoginScr.imgTitle) / 10));
            g.drawRegion(LoginScr.imgTitle, 0, y2, mGraphics.getImageWidth(LoginScr.imgTitle), mGraphics.getImageHeight(LoginScr.imgTitle) / 10, 0, x, 0 + (mGraphics.getImageHeight(LoginScr.imgTitle)/10)-5, mGraphics.BOTTOM | mGraphics.HCENTER);
        }
    ...
    public Image[] loading = new Image[7];
    public Image img_wait;
    public void paintLoading(mGraphics g, bool isLoad)
    {
        try
        {
            int x = GameCanvas.w / 2;
            int y = GameCanvas.h / 2;
            for (int i = 0; i < loading.Length; i++)
            {
                if (loading[i] == null)
                {
                    loading[i] = loadImage($"/inovar/img_loading_{i}.png");
                }
            }
            g.drawImage(loading[gameTick / 4 % 7], x, y, mGraphics.VCENTER | mGraphics.HCENTER);
            if (img_wait == null)
            {
                img_wait = loadImage("/inovar/img_wait.png");
            }
            if (isLoad)
            {
                g.drawImage(img_wait, x, y, mGraphics.VCENTER | mGraphics.HCENTER);
            }
        }
        catch (Exception e)
        {
    ...
    public static void resizeImage(Image img,int wx1 , int hx1)
    {
    ...
    public static void clearAllPointerEvent()
    {
        ...
        Debug.LogError("clearAllPointerEvent");
    }
