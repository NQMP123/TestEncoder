...
public class mFont
{
    ...
    public string[] splitFontArray(string src, int lineWidth, int font)
    {
        MyVector myVector = splitFontVector(src, lineWidth, font);
        string[] array = new string[myVector.size()];
        for (int i = 0; i < myVector.size(); i++)
        {
            array[i] = (string)myVector.elementAt(i);
    ...
    public MyVector splitFontVector(string src, int lineWidth, int font)
    {
        MyVector myVector = new MyVector();
        string text = string.Empty;
        for (int i = 0; i < src.Length; i++)
        {
            ...
            text += src[i];
            if (getWidthWithFont(text, font) > lineWidth)
            {
                int num = 0;
                num = text.Length - 1;
                while (num >= 0 && text[num] != ' ')
    ...
    private int getWidthWithFont(string s, int font)
    {
        // Sử dụng một GUIStyle mới với kích thước font
	...
	public void _drawString(mGraphics g, string st, int x0, int y0, int align,int font)
	{
        y0 += yAddFont;
        GUIStyle gUIStyle = new GUIStyle(GUI.skin.label);
        gUIStyle.font = myFont;
		gUIStyle.fontSize = font * mGraphics.zoomLevel;
