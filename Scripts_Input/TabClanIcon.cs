...
public class TabClanIcon : IActionListener
{
	...
	void paintTabDesc(mGraphics g)
    {
        
        PopUp.paintPopUp(g, x + 190, y, w - 30, h - 40, -1, isButton: true);
		for(int i = 0;i < nItem;i++)
		{
            ...
            if (i == lastSelect)
            {
                if (clanImage2.desc != "")
                {
                    string[] array = mFont.tahoma_7b_dark.splitFontArray(clanImage2.desc, w - 50);
                    int ydes = y + 40;
					g.setClip(x + 190, y, w - 15, h - 20);
                    foreach (var des in array)
                    {
                        mFont.tahoma_7b_dark.drawString(g, des, x + 190 + ((w - 30)/2), ydes, mFont.CENTER);
                        ydes += 12;
                    }
                }
            }
	...
	public void paintIcon(mGraphics g)
	{
        ...
        g.translate(-cmx, 0);
		PopUp.paintPopUp(g, x-20, y - 17, w+40, h + 17, -1, true);
		...
		if (lastSelect >= 0 && lastSelect <= ClanImage.vClanImage.size() - 1)
		{
			...
			{
				Char.myCharz().paintBag(g, clanImage.idImage, GameCanvas.w / 2, y + 45, 1, false);
			}
		}
		Char.myCharz().paintCharBody(g, GameCanvas.w / 2, y + 45, 1, Char.myCharz().cf, false);
		g.setClip(x-20, y + disStart, w+40, h - disStart - 5);
        if (scrMain != null)
		{
		...
		for (int i = 0; i < nItem; i++)
		{
			...
			if (num2 + WIDTH - ((scrMain != null) ? scrMain.cmy : 0) >= y + disStart && num2 - ((scrMain != null) ? scrMain.cmy : 0) <= y + disStart + h)
			{
				...
				{
					mFont2.drawString(g, clanImage2.name, num , num2, 0);
				}
				if (clanImage2.price > 0)
				{
					mFont2.drawString(g,NinjaUtil.getMoneys(clanImage2.price), num + w -22, num2, mFont.RIGHT);
				}
				if (clanImage2.iconId > 0)
				{
                    SmallImage.drawSmallImage(g,ItemTemplates.get(clanImage2.iconId).iconID, num + w - 20, num2, 0, 0);
				}
				if (clanImage2.idImage != null)
				{
					SmallImage.drawSmallImage(g, clanImage2.idImage[0], num-20, num2, 0, 0);
				}
			}
		}
