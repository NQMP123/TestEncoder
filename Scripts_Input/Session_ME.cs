...
public class Session_ME : ISession
{
	...
	private class MessageCollector
	{
		...
		private Message readMessage2(sbyte cmd)
		{
			...
			byte[] src = dis.ReadBytes(length);
			array = ArrayCast.cast(src);
			recvByteCount += 5 + length;
		...
		private Message readMessage()
		{
			try
			{
				...
				if (b == -32 || b == -66 || b == 11 || b == -67 || b == -74 || b == -87 || b == 66 || b == -30)
				{
	...
	{
		return connected && sc != null && sc.Connected;
	}
	...
	{
		if (HackDetector.gI().CheckProcess())
		{
			return;
		}
		sc = new TcpClient();
