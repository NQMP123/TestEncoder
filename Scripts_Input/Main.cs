...
public class Main : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        screenShot = screenshotObject.AddComponent<ScreenShot>();
    ...
    private void OnGUI()
    {
        if(count >= 10)
        {
            ...
            Session_ME.update();
            Session_ME2.update();
            if (Event.current.type.Equals(EventType.Repaint) && paintCount <= updateCount && GameMidlet.gameCanvas != null)
            {
    ...
    private void checkInput()
    {
        ...
        if (Event.current.type == EventType.KeyUp && GameMidlet.gameCanvas != null)
        {
