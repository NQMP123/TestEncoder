...
public class Waypoint : IActionListener
{
	...
	{
		if (this.isEnter && mSystem.currentTimeMillis() - lastPaint >= 250L)
		{
			ServerEffect.addServerEffect(134, (minX + maxX) / 2, LoadMap.GetYGround((minX + maxX) / 2), 3) ;
            ServerEffect.addServerEffect(134, (minX + maxX) / 2, LoadMap.GetYGround((minX + maxX) / 2), 3);
            lastPaint = mSystem.currentTimeMillis();
        }
    }
